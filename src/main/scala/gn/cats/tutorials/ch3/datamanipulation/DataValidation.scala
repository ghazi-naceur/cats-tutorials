package gn.cats.tutorials.ch3.datamanipulation

import cats.kernel.Semigroup

import scala.annotation.tailrec
import scala.util.Try

// 5
object DataValidation {

  import cats.data.Validated
  val aValidCValue: Validated[String, Int] = Validated.Valid(12) // "right" value
  val anInvalidCValue: Validated[String, Int] = Validated.invalid("Something went wrong") // "left" value

  val aTest: Validated[String, Int] = Validated.cond(12 > 39, 99, "12 is inferior than 39")
  // "99" will be return if predicate is true
  // "12 is inferior than 39" will be return if predicate is false

  def testPrime(n: Int): Boolean = {
    @tailrec
    def tailRecPrime(d: Int): Boolean =
      if (d <= 1) true
      else n % d != 0 && tailRecPrime(d - 1)

    if (n == 0 || n == 1 || n == -1) false
    else tailRecPrime(Math.abs(n / 2))
  }

  // Exercise:
  /*
    - n must be a prime
    - n must be non-negative
    - n <= 100
    - n must be even
   */
  def testNumber(n: Int): Either[List[String], Int] = {
    val isNotEven: List[String] = if (n % 2 == 0) List() else List("Number must be even")
    val isNegative: List[String] = if (n >= 0) List() else List("Number must be non-negative")
    val isTooBig: List[String] = if (n <= 100) List() else List("Number must be less than or equal to 100")
    val isNotPrime: List[String] = if (testPrime(n)) List() else List("Number must be a prime")

    if (n % 2 == 0 && n >= 0 && n <= 100 && testPrime(n)) Right(n)
    else Left(isNotEven ++ isNegative ++ isTooBig ++ isNotPrime)
  }
  // In the last example, we're testing each validation separately. With Cats, we can combine all these validations:

  // We need a Semigroup of a List[String] and a Semigroup of Int, and that's because '.combine' will need the default
  // for combination function of both: left ans right side of the Validated instances:
  import cats.instances.list._
  implicit val combineIntMax: Semigroup[Int] = Semigroup.instance[Int](Math.max)

  def validateNumber(n: Int): Validated[List[String], Int] =
    Validated
      .cond(n % 2 == 0, n, List("Number must be even"))
      .combine(Validated.cond(n >= 0, n, List("Number must be non-negative")))
      .combine(Validated.cond(n <= 100, n, List("Number must be less than or equal to 100")))
      .combine(Validated.cond(testPrime(n), n, List("Number must be a prime")))

  // Chain validations:
  aValidCValue.andThen(v => anInvalidCValue)
  // Validated is not applicable for a Monad

  // Test a valid value:
  aValidCValue.ensure(List("Something went wrong"))(_ % 2 == 0)
  // The value 'aValidCValue' will be tested against the predicate '(_ % 2 == 0)', and it's return false, the value will
  // be invalid with an updated value 'List("Something went wrong")'

  // Transform:
  aValidCValue.map(_ + 1) // right side
  aValidCValue.leftMap(_.length) // list side
  aValidCValue.bimap(_.length, _ + 1) // right and left side

  // Interoperate with stdlib:
  val eitherToValidated: Validated[List[String], Int] = Validated.fromEither(Right(12))
  val optionToValidated: Validated[List[String], Int] = Validated.fromOption(None, List("Nothing present here"))
  val tryToValidated: Validated[Throwable, Int] = Validated.fromTry(Try("something".toInt))

  // Backwards:
  aValidCValue.toOption
  aValidCValue.toEither

  // Exercise: Form validation
  object FormValidation {
    import cats.instances.string._
    type FormValidation[T] = Validated[List[String], T]

    def getValue(form: Map[String, String], fieldName: String): FormValidation[String] =
      Validated.fromOption(form.get(fieldName), List(s"The field '$fieldName' must be specified"))

    def nonBlank(value: String, fieldName: String): FormValidation[String] =
      Validated.cond(value.nonEmpty, value, List(s"The field '$fieldName' must not be blank"))

    def emailProperForm(email: String): FormValidation[String] =
      Validated.cond(email.contains("@"), email, List("Email is invalid"))

    def passwordCheck(password: String): FormValidation[String] =
      Validated.cond(password.length >= 10, password, List("Password must at least 10 characters long"))

    /*
      Fields are: name, email, password
      Rules are:
        - name, email and password must be specified
        - name must not be blank
        - email must have @
        - password must have at least characters
     */
    def validatedForm(form: Map[String, String]): FormValidation[String] =
      getValue(form, "Name")
        .andThen(name => nonBlank(name, "Name"))
        .combine(getValue(form, "Email").andThen(emailProperForm))
        .combine(getValue(form, "Password").andThen(passwordCheck))
        .map(_ => "User registration complete")
  }

  // Extension methods:
  import cats.syntax.validated._
  val aValid: Validated[List[String], Int] = 12.valid[List[String]]
  val anError: Validated[String, Int] = "Something went wrong".invalid[Int]

  def main(args: Array[String]): Unit = {
    val form1 = Map("Name" -> "Isaac", "Email" -> "isaac@hxh.com", "Password" -> "netero125hxh")
    println(FormValidation.validatedForm(form1))

    val form2 = Map("Name" -> "", "Email" -> "hxh.com", "Password" -> "123")
    println(FormValidation.validatedForm(form2))
  }
}
